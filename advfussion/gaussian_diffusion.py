...
from advfussion.nn import mean_flat
from advfussion.losses import normal_kl, discretized_gaussian_log_likelihood
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.loss_type = loss_type#gd.LossType.MSE  
        self.rescale_timesteps = rescale_timesteps# False

        # Use float64 for accuracy.
        betas = np.array(betas, dtype=np.float64) # (βt)^2 
        ''''
       ...
       0.01982072, 0.01984064, 0.01986056, 0.01988048, 0.0199004 ,
        ...
        alphas = 1.0 - betas # (αt)^2
        self.alphas_cumprod = np.cumprod(alphas, axis=0) # (bar αt)^2
        self.alphas_cumprod_prev = np.append(1.0, self.alphas_cumprod[:-1]) # (bar αt-1)^2
        self.alphas_cumprod_next = np.append(self.alphas_cumprod[1:], 0.0) # (bar αt)^2
        assert self.alphas_cumprod_prev.shape == (self.num_timesteps,)

        # calculations for diffusion q(x_t | x_{t-1}) and others
        self.sqrt_alphas_cumprod = np.sqrt(self.alphas_cumprod) # (bar αt)
        self.sqrt_one_minus_alphas_cumprod = np.sqrt(1.0 - self.alphas_cumprod)# (1 - (bar αt)^2) **0.5
        self.log_one_minus_alphas_cumprod = np.log(1.0 - self.alphas_cumprod) # log(1 - (bar αt)^2)
