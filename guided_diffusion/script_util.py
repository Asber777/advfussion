...
def get_steps_scale_map(num_timesteps, section_counts, scales):
        ...
        if section_counts.startswith("ddim"):
            ...
            for i in range(1, num_timesteps):
                if len(range(0, num_timesteps, i)) == desired_count:
                    for j, t in enumerate(range(0, num_timesteps, i)):
                        steps_scale_map[t] = scales[j] if scales else 1
    ...
    for i, section_count in enumerate(section_counts):
        ...
        for _ in range(section_count):
            steps_scale_map[start_idx + round(cur_idx)] = scales[i] if scales else 1
            cur_idx += frac_stride
        start_idx += size
    return steps_scale_map
...
def pgd(x, y, attack_model, nb_iter=1, eps=4./255, eps_iter=1./255, 
    ...
    with th.enable_grad():
        ...
        for _ in range(nb_iter):
            ...
            grad_sign *= eps_iter
            delta.data += grad_sign  if not target else - grad_sign 
...
def arr2pic_save(arr, logdir, SHOW_PIC_N=5, name="result.jpg", trans=True):
    ...
    if trans: picture = th.clamp((picture + 1) / 2.0, 0., 1.)
