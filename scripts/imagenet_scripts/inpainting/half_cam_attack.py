...
def main():
    ...
    def cond_fn(x, t, y=None, guide_x=None, guide_x_t=None, 
        ...
        Input explain:
        x: x_t 
        mean: x_{t-1}(fron backward process of DDPM)
        variance/ log_variance: from backward process of DDPM
        pred_xstart: pred_xstart from backward process of DDPM
        t: t, time in "x_t"
        '''
        '''
        Inpainting:
        x_{t-1} = m \times x^{obj}_{t-1} + \hat{x_{t-1}}
        \hat{x_{t-1}} = (1-m) * mean + (1-m)* delta 
        '''
        ...
        if args.use_adver:
            ...
            with th.enable_grad():
                ...
                for _ in range(args.nb_iter_conf):
                    ...
                    delta.data += grad_  * args.adver_scale  * (1-mask) 
                    delta.data = clamp(delta.data, -eps, eps)
                    # delta.data = clamp(pred_xstart.data + delta.data, -1, +1) - pred_xstart.data
                    delta.grad.data.zero_()
            # mean = mean.float() + delta.data.float()  # original 
            if args.range_t2_s <=time <= args.range_t2_e:
                mean = mask *  guide_x_t  + (1-mask) * mean.float()  + delta.data.float()  # inpainting
            else:
